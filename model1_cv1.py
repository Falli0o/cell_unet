...
data_filenames = sorted(f1_data_filenames) + sorted(f2_data_filenames) + sorted(f3_data_filenames) + sorted(f4_data_filenames) + sorted(f5_data_filenames)
label_filenames = sorted(f1_label_filenames) + sorted(f2_label_filenames) + sorted(f3_label_filenames) + sorted(f4_label_filenames) + sorted(f5_label_filenames)
...
hist = model.fit(X_train, Y_train,  batch_size=32,epochs=200,callbacks=[model_checkpoint])#validation_split=0.1,
...
f = open(r'/content/gdrive/My Drive/ColabData/final_dataset/f1_hist.txt','w')
...
imsave('f1_re.png',re[0].reshape(256,256));
files.download('f1_re.png')
